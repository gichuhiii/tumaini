...
// Add jsPDF download logic
const getRiskLevelColorRGB = (level: string) => {
...
const downloadPDF = async (result: RiskResult, form: RiskAssessmentData) => {
  ...
  let y = 20;
  doc.setFontSize(18);
  doc.text('Risk Assessment Results', 20, y);
  y += 10;
  doc.setFontSize(12);
  ...
  const [r, g, b] = getRiskLevelColorRGB(result.risk_level);
  doc.setTextColor(r, g, b);
  doc.text(`Risk Level: ${result.risk_level.toUpperCase()}`, 20, y);
  doc.setTextColor(0, 0, 0); // Reset to black
  y += 8;
  doc.text(`What This Means: ${riskInfo.description}`, 20, y, { maxWidth: 170 });
  y += 16;
  doc.setFontSize(14);
  doc.text('Recommended Next Steps:', 20, y);
  y += 8;
  doc.setFontSize(12);
  doc.text(`Next Follow-up: ${riskInfo.followUp}`, 20, y);
  y += 8;
  doc.text('Recommended Services:', 20, y);
  y += 8;
  riskInfo.services.forEach((service: string) => {
  ...
  if (result.recommendations.length > 0) {
    y += 4;
    doc.setFontSize(14);
    doc.text('Additional Recommendations:', 20, y);
    y += 8;
    doc.setFontSize(12);
  ...
  y += 4;
  doc.setFontSize(14);
  doc.text('Support & Resources:', 20, y);
  y += 8;
  doc.setFontSize(12);
  doc.text(riskInfo.supportMessage, 20, y, { maxWidth: 170 });
  y += 14;
  doc.setFontSize(10);
  doc.text('You can discuss these results with your healthcare provider. Early detection is key to good health outcomes.', 20, y, { maxWidth: 170 });
  y += 10;
  doc.setFontSize(10);
  doc.text('Generated by tumAIni - Cervical Cancer Screening Platform', 20, 285);
  doc.save('risk-assessment-results.pdf');
};
