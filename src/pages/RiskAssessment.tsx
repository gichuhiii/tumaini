...
const RiskAssessment = () => {
  ...
  const validate = () => {
    const errs: any = {};
    if (!form.patientId) errs.patientId = "Required";
  ...
  return (
    <div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {/* Patient Form */}
        <form className="bg-white rounded-lg border p-6 space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input name="patientId" value={form.patientId} onChange={handleChange} className="w-full border rounded px-3 py-2" />
              {errors.patientId && <div className="text-xs text-red-500">{errors.patientId}</div>}
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Age</label>
              <input name="age" value={form.age} onChange={handleChange} type="number" className="w-full border rounded px-3 py-2" />
          ...
          <Button type="submit" className="w-full bg-pink-500 hover:bg-pink-600 text-white mt-4">{loading ? "Assessing..." : "Assess Risk"}</Button>
        </form>
        {/* Results Panel */}
        {result && !loading && (
